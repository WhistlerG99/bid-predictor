...
def get_output_dir():
    ...
    get_output_dir = os.environ.get("CATBOOST_TRAIN_DIR", "/opt/ml/output/catboost")
    try:
        os.makedirs(get_output_dir, exist_ok=True)
    except PermissionError:
        # Fall back to /tmp to avoid failing the whole job
        get_output_dir = os.path.join("/tmp", "catboost")
        os.makedirs(trainget_output_dir_dir, exist_ok=True)
        print(f"WARNING: /opt/ml/output not writable; using {get_output_dir}")
